...
export default function DashboardEditor({ user }) {
    ...
    async function handleProfileSubmit(e) {
        ...
        try {
            const res = await fetch("/api/auth/profile", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(profileData) });
            if (res.ok) showToast("Profile updated!"); else showToast("Failed", "error");
        } catch (e) { showToast("Error", "error"); } finally { setSavingProfile(false); }
    }
            ...
            <main className="max-w-7xl mx-auto p-4 lg:p-8">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        <div className="bg-emerald-500 text-white p-4 rounded-xl font-bold text-lg mb-6 shadow-xl animate-pulse">
                            ✅ DASHBOARD EDITOR LOADED <br />
                            User: {user?.username} <br />
                            Links Sent: {links?.length}
                        </div>
                            ...
                            <div className="space-y-8">
                                ...
                                <section className="bg-slate-800 p-6 rounded-2xl border border-slate-700">
                                    ...
                                    <div className="flex flex-col md:flex-row items-center gap-8">
                                        ...
                                        <div className="flex-1 w-full space-y-4">
                                            ...
                                            <button onClick={handleProfileSubmit} className="bg-slate-700 hover:bg-slate-600 text-white px-6 py-2 rounded-lg text-sm font-bold transition-colors">
                                ...
                                <section className="bg-slate-800 p-6 rounded-2xl border border-slate-700">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                        {Object.entries(themePresets).map(([key, theme]) => (
                                            <button
                                                key={key}
                                                onClick={() => { setProfileData(prev => ({ ...prev, themePreset: key })); handleProfileSubmit({ preventDefault: () => { } }); }}
                                                className={`relative aspect-[3/4] rounded-xl overflow-hidden border-2 transition-all ${profileData.themePreset === key ? "border-indigo-500 scale-105 shadow-xl shadow-indigo-500/20" : "border-slate-700 hover:border-slate-500"}`}
                                            >
                                                <div className={`absolute inset-0 ${theme.bg}`}></div>
                                                <div className="absolute inset-2 bg-white/10 rounded-lg flex flex-col items-center justify-center gap-2">
                                                    <div className={`w-3/4 h-2 rounded-full ${theme.cardBg}`}></div>
                                                    <div className={`w-3/4 h-2 rounded-full ${theme.cardBg}`}></div>
                                                </div>
                                                <div className="absolute bottom-0 w-full bg-black/50 p-2 text-center text-xs font-bold text-white backdrop-blur-sm">
                                                    {theme.name}
                                                </div>
                                            </button>
                                        ))}
                                    ...
                                    <div className="mt-8">
                                        ...
                                        <div className="flex flex-wrap gap-4">
                                            {Object.keys(buttonStyles).map(style => (
                                                <button
                                                    key={style}
                                                    onClick={() => { setProfileData(prev => ({ ...prev, buttonStyle: style })); handleProfileSubmit({ preventDefault: () => { } }); }}
                                ...
                                <section className="bg-slate-800 p-6 rounded-2xl border border-slate-700">
                                    ...
                                    <div className="flex items-center gap-4">
                                        {profileData.customBackground ? (
                                            <div className="w-24 h-48 bg-slate-900 rounded-lg overflow-hidden border border-slate-600 relative group">
                                                ...
                                                <button
                                                    onClick={() => { setProfileData(p => ({ ...p, customBackground: null })); handleProfileSubmit({ preventDefault: () => { } }); }}
                                    ...
                                    <div className="mt-6 flex items-center justify-between p-4 bg-slate-900/50 rounded-xl border border-slate-700">
                                        ...
                                        <button
                                            onClick={() => { setProfileData(p => ({ ...p, enableAmbient: !p.enableAmbient })); handleProfileSubmit({ preventDefault: () => { } }); }}
                                        ...
                                        {['instagram', 'youtube', 'spotify', 'tiktok'].map(platform => (
                                            <div key={platform} className="relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-slate-900 border border-slate-600 rounded-lg py-3 pl-20 pr-4 text-white focus:outline-none focus:border-indigo-500 font-mono text-sm"
                                                    placeholder="Username only"
                                                />
                                            </div>
                                        ))}
                                        <button onClick={handleProfileSubmit} className="mt-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 rounded-xl transition-all w-full">
                                            Save Socials
                                        </button>
                                    </div>
                                </section>
                        ...
                        <div className="sticky top-24">
                            ...
                            <div className="mockup-phone border-slate-800 bg-slate-900 rounded-[3rem] border-[14px] overflow-hidden shadow-2xl h-[calc(100vh-160px)] relative">
                                <div className="absolute top-0 w-full h-8 bg-black z-20 flex justify-center">
                                    <div className="w-1/3 h-5 bg-black rounded-b-2xl"></div>
                                </div>
                                <div className="h-full w-full overflow-y-auto overflow-x-hidden no-scrollbar bg-black">
                                    <ProfileClient user={userForPreview} />
                                </div>
                            </div>
                        </div>
                    </div>
